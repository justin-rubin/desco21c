...
export function initEntities(state) {
    ...
    state.clients = [
        new ClientTrophy(300, GROUND_LEVEL - CLIENT_SIZE.h / 2, state.assets),
        // … more trophies
    ];
    state.coffeeMugs = [new Coffee(1200, GROUND_LEVEL - CLIENT_SIZE.h, state.assets)];
    state.macErrors = [new MacError(700, GROUND_LEVEL - PLAYER_SIZE.h, state.assets), new MacError(1000, GROUND_LEVEL - PLAYER_SIZE.h, state.assets)];
    // state.zombies = [new Zombie(500, GROUND_LEVEL - PLAYER_SIZE.h, { minX: 400, maxX: 800 }, state.assets)];
    state.player = new Player(state.assets);
    state.levelEndX = state.platforms[0].width - PLAYER_SIZE.w + 480;
